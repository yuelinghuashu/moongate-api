...
- **Better Comments 预设**：官方配色**内置**主题（零配置开箱即用），另有 `extras/better-comments.json` 独立预设供单独使用。两条通道都从语义层生成，不会产生配色漂移。
- **终端 ANSI 色同步**：16 色 ANSI 配色映射到主题语义色，消除编辑器与终端的视觉割裂。
- **跨平台令牌资产**：通过自动生成的 `moongate-colors.css`、`_tokens.scss`、`tokens.ts`，供博客、文档站、UI 组件库直接引用——一套颜色贯穿所有产品。
...
- **基础篇**：你从手动 JSON 出发，创建并发布了第一个 VS Code 主题，掌握了核心机制与发布流程。
- **工程篇**：你通过 YAML 模块化 + 构建脚本，让主题变得可维护、可自动构建。
- **设计系统篇**：你用 DTCG 三层架构管理颜色，通过语义层与重力补偿构建了昼夜双变体。
- **工业级篇**：你构建了可测试、可验证的构建脚本架构，让质量保障自动化。
- **品牌生态篇**：你将主题升华为设计系统，用设计哲学、视觉契约和跨平台资产建立了完整的品牌生态。
...
_本文是 VS Code 主题开发系列的品牌生态篇。五篇连读，助你从零成长为设计系统工程师。_
